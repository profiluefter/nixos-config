{ ... }:
{
  flake.nixosModules.users =
    {
      config,
      lib,
      options,
      ...
    }:
    {
      users.mutableUsers = false;

      users.users.root.hashedPasswordFile = lib.mkIf (options ? sops) config.sops.secrets.rootHash.path;

      users.users.fabian = {
        # enable = true;
        uid = 31337;
        isNormalUser = true;
        linger = true;
        description = "Fabian Gurtner";
        extraGroups = [
          "wheel"
          "docker"
        ];

        hashedPasswordFile = lib.mkIf (options ? sops) config.sops.secrets.userHash.path;

        openssh.authorizedKeys.keys = [
          "ssh-rsa "
        ];
      };

      security.sudo.extraConfig = ''
        Defaults lecture = never
      '';
    };
}
